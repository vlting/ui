# Component Contract — MonetizedContentLock

## 1. Public API

### Base Props

`MonetizedContentLock` is implemented as a Tamagui `styled(YStack)` and therefore accepts all Tamagui / React Native `View` props, including:
...
- In `locked` state: overlay visible; content beneath is obfuscated.
- In `unlocked` state: overlay is not rendered; content is shown normally.
- In `loading` state: while access check is pending, a skeleton or spinner is shown in place of the CTA.
- In `error` state: if the access check fails, an error message is shown with a retry option.
...
- Removing a named static sub-component (e.g., `MonetizedContentLock.Item`).
