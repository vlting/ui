# Component Contract — Banner

## 1. Public API

### Base Props

`Banner` is implemented as a Tamagui `styled(YStack)` and therefore accepts all Tamagui / React Native `View` props, including:
...
Intended to be wrapped by: page layout roots, app shell containers, section headers.

May contain: an icon slot, a message text slot, an action slot, a dismiss slot.
...
- Removing a named static sub-component (e.g., `Banner.Item`).
