# Component Contract — ChatLayout

## 1. Public API

### Base Props

`ChatLayout` is implemented as a Tamagui `styled(YStack)` and therefore accepts all Tamagui / React Native `View` props, including:
...
- In `sidebar visible` state: two-panel layout; sidebar shows conversation list.
  - **sidebar collapsed** (mobile): full-width thread is shown; sidebar accessible via navigation gesture or button.
- In `loading` state: skeleton placeholders fill sidebar and thread.
- In `empty thread` state: empty state message shown in the thread area.
...
- Removing a named static sub-component (e.g., `ChatLayout.Item`).
