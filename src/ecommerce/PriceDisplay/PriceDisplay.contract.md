# Component Contract — PriceDisplay

## 1. Public API

### Base Props

`PriceDisplay` is implemented as a Tamagui `styled(YStack)` and therefore accepts all Tamagui / React Native `View` props, including:
...
Intended to be wrapped by: ProductCard.Price, CartItemRow price area, BillingSummary rows, VariantSelector option labels, OrderSummaryCard items.

May contain: the formatted current price string, an optional struck-through original price, and an optional discount label.
...
- Removing a named static sub-component (e.g., `PriceDisplay.Item`).
