...
- Should NOT be used purely for visual sizing without heading semantics; the `accessibilityRole: 'header'` is always applied.
...
- Layout rules: renders inline as a Tamagui Text element. Does not impose block-level layout or margin. Consumers must manage spacing around headings.
...
- Screen reader behavior: announced as a heading due to `accessibilityRole: 'header'`. Screen reader users rely on headings for page navigation. Consumers should set `aria-level` to communicate the semantic level (1-6) independently of visual size.
...
- ARIA requirements:
  - `accessibilityRole: 'header'` is always set.
  - `aria-level` is NOT set automatically. Consumers must set it when semantic heading level matters for assistive technology navigation (e.g., `aria-level={1}` for the page's primary heading).
...
- What must be tested:
  - Each level (1-6) renders with the correct font size, line height, and font weight tokens.
  - Default level is 2 when no `level` prop is provided.
  - `accessibilityRole` is set to `'header'` on all instances.
  - Uses `$heading` font family and `$color` text color.
  - Consumer-applied `aria-level` is forwarded correctly.
- Interaction cases: not applicable (non-interactive).
- Accessibility checks:
  - Screen readers announce the element as a heading.
  - `aria-level` is respected when set by the consumer.
