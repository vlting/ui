 ...
 */
export function generateStaticPropsCss<K extends FilterKeys>(
  ...
  function values<P extends StlPropKey, V extends { [k: string]: unknown } = Placeholder>(
    ...
    const baseValues =
      inheritedProps[prop as keyof typeof inheritedProps] ?? DEFAULT_VALUES
    const valuesMap = { ...baseValues, ...(customValues ?? {}) }

    type BaseType = P extends keyof typeof inheritedProps
      ? typeof INHERITED_VALUES
      : typeof DEFAULT_VALUES
    type ValueKey = V extends Placeholder ? keyof BaseType : keyof BaseType | keyof V

    return Object.keys(valuesMap).reduce(
      (output: { [k in ValueKey]: string }, key: string) => {
  ...
  const props = {
    ...
    animationIterationCount: values(
