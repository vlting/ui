...
export const nthChildCheckers = {
  ...
  [BASE]: (index: number, nthChildKey: NthChildKeys) => {
    ...
    if (parts.length === 1) {
      return index === Number.parseInt(nthValue, 10) - 1
    }
    if (parts[0] === '0') {
      return index === Number.parseInt(parts[1], 10) - 1
    }
    ...
    if (parts[0] === '-') {
      ...
      nMultiplier = Number.parseInt(parts[0], 10)
    }
    ...
    const nModifier = Number.parseInt(parts[1], 10)
