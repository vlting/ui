...
export function getFontFamily(hash: CharHash) {
  const baseVars = {
    ...
    ...enumKeys(BodyFontFamily).reduce(
      (
        output: Record<keyof typeof BodyFontFamily, ScaleEntry>,
        key: keyof typeof BodyFontFamily,
      ) => {
    ...
    ...enumKeys(HeadingFontFamily).reduce(
      (
        output: Record<keyof typeof HeadingFontFamily, ScaleEntry>,
    ...
    ...enumKeys(CodeFontFamily).reduce(
      (
        output: Record<keyof typeof CodeFontFamily, ScaleEntry>,
        key: keyof typeof CodeFontFamily,
      ) => {
  ...
  const sharedVars = {
    ...
    body: {
      ...hash.var,
      value: `${baseVars[DEFAULT_FONTS.body].ref}, ${baseVars.systemSans.ref}`,
    },
    button: {
      ...hash.var,
