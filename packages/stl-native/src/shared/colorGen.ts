...
const AVAILABLE_HUES = [
  8, 33, 45, 77, 123, 138, 174, 193, 194, 222, 264, 267, 313, 350,
] as const
 ...
 */
function findBracketingHues(targetHue: number): {
  lowHue: number
  highHue: number
  t: number
} {
...
function interpolateSets(
  ...
  COLOR_KEYS.forEach((key) => {
    ...
    const startLuminance = isStartSetZeroSat
      ? decodeValue(startPair[0])
      : decodeValue(startPair[1])
...
export function getTextColor(
  ...
  return (
    typeof targetKey === 'number' ? `${targetColor}${targetKey}` : targetKey
  ) as ThemeColor
}
...
export function generateThemeColors<T = string>(
  ...
  Object.entries(inputColors).forEach(([colorKey, inputValue]) => {
    ...
    saturation =
      isNeutral && saturation > 40 ? Math.round((saturation / 100) * 40) : saturation
    ...
    if (isMapped) {
      ...
      setValue(
        colorKey as keyof ColorPalette<T>,
        colors,
        getHSL(hue, saturation, isDark ? contrast : 100 - contrast),
      )
    } else {
      const fullSat = isNeutral ? MAX_NEUTRAL_SATURATION : 100
      ...
      const needsInterpolation =
        saturation !== 0 && saturation !== fullSat && saturation !== halfSat
      ...
      const alphaColorKey =
        AlphaColorName[`${colorKey}${ALPHA_KEY}` as keyof typeof AlphaColorName]
      if (alphaColorKey) {
        const minLuminance = isDark ? 0 : 100
        const maxLuminance = isDark ? 100 : 0
        const luminance =
          colorKey === 'min' ? minLuminance : colorKey === 'max' ? maxLuminance : 50
        ALPHA_VALUES.forEach((alpha, index) => {
          const alphaKey = (index + 1) as ColorNumberKey
          setValue(
            numerize(alphaColorKey, alphaKey),
            colors,
            getHSL(hue, saturation, luminance, alpha),
            alphaKey,
          )
        })
      }
