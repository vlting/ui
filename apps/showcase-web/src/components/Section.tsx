...
export function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section style={{ marginBottom: 48 }}>
      ...
      <hr style={{ border: 'none', borderTop: '1px solid var(--stl-borderColor, #e5e5e5)', marginBottom: 16 }} />
...
export function DemoCard({ label, children }: { label: string; children: ReactNode }) {
  return (
    ...
    >
      <div style={{ fontSize: 12, fontWeight: 600, color: '#888', marginBottom: 12, textTransform: 'uppercase', letterSpacing: 1 }}>
        {label}
      </div>
      {children}
    </div>
