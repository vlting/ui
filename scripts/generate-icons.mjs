...
function main() {
  ...
  const categoriesDir = join(ICONS_OUTPUT, 'categories')
  rmSync(categoriesDir, { recursive: true, force: true })
  mkdirSync(categoriesDir, { recursive: true })

  // Group components by category
  const componentsByCategory = new Map()
  for (const icon of allComponents) {
    if (!componentsByCategory.has(icon.category)) {
      componentsByCategory.set(icon.category, [])
    }
    componentsByCategory.get(icon.category).push(icon)
  }

  for (const cat of sortedCategories) {
    const components = componentsByCategory.get(cat) || []
    const catLines = [
      '// This file is auto-generated by scripts/generate-icons.mjs',
      '// Do not edit manually.',
      '',
  ...
  for (const cat of sortedCategories) {
    catIndexLines.push(`export * from './${cat}'`)
  }
  catIndexLines.push('')
