# Task 006 — Crushd: Dating Example App (Purple Brand + Full Implementation)
...
3. `src/dating/` contains implemented components (from task 005) — not just stubs
4. Read `AI_CONSTITUTION.md` and `DESIGN_CONSTITUTION.md` at repo root before making changes

If task 004 or 005 are incomplete, the app skeleton or components may still be stubs. Adapt accordingly — you can build pages using `@vlting/ui/primitives`, `@vlting/ui/layout`, and `@vlting/ui/forms` as fallbacks where dating-specific components aren't ready yet.

---

## Part 1 — Rebrand: "Kinship" → "Crushd"

### App Name: **Crushd**
### Tagline: *"Your next crush is closer than you think."*

Update all references across the example app:
- `examples/dating/package.json` → name: `@vlting/examples-crushd`
- `examples/dating/index.html` → title: `Crushd — Your next crush is closer than you think`
- `examples/dating/src/theme/datingBrand.ts` → rename to `crushdBrand.ts`, brand name: `crushd`
- `examples/dating/src/main.tsx` → update import path
- Any text/labels in router, layout, or pages that say "Kinship" → "Crushd"
...
## Part 2 — Purple Brand Theme (`crushdBrand.ts`)

Replace the coral/warm brand from task 004 with a purple-forward theme. Follow the `voltaBrand.ts` pattern exactly.

### Neutral Palette — Warm Mauve (pink-gray undertone)

**Light neutral (12 steps):**
```
#faf8f9   — 1  near-white with subtle pink warmth
#f3eff1   — 2  very light mauve
#e8e2e6   — 3  light mauve
#dbd3d8   — 4  medium-light mauve
#c9bfc6   — 5  medium mauve
#afa3ac   — 6  medium-strong mauve
#8f8390   — 7  mauve
#706474   — 8  dark mauve
#544959   — 9  darker mauve
#3d3342   — 10 very dark mauve
#28202d   — 11 near-black mauve
#17111b   — 12 darkest mauve
```

**Dark neutral (12 steps) — inverted order of light, adjusted for dark mode readability:**
```
#17111b   — 1  near-black
#1f1825   — 2  very dark mauve
#28202d   — 3  dark mauve
#332a39   — 4  dark mauve
#3f3547   — 5  medium-dark mauve
#504659   — 6  medium mauve
#665a6d   — 7  medium-light mauve
#7e7285   — 8  lighter mauve
#9c90a3   — 9  light mauve
#bcb2c1   — 10 very light mauve
#dad2de   — 11 near-white mauve
#f0ecf2   — 12 near-white
```
...
```
#f5f0ff   — 1  near-white purple tint
#ede5ff   — 2  very light purple
#d8c8ff   — 3  light purple
#c0a5ff   — 4  medium-light purple
#a67fff   — 5  medium purple
#8b5cf6   — 6  medium-strong purple
#7c3aed   — 7  electric purple (PRIMARY BRAND COLOR)
#6d28d9   — 8  deep purple
#5b21b6   — 9  deeper purple
#4c1d95   — 10 very deep purple
#3b0f7a   — 11 darkest purple
#2a0860   — 12 near-black purple
```

**Dark purple (12 steps) — inverted:**
```
#2a0860   — 1
#3b0f7a   — 2
#4c1d95   — 3
...
#6d28d9   — 5
#7c3aed   — 6
#8b5cf6   — 7  electric purple (primary in dark mode)
#a67fff   — 8
#c0a5ff   — 9
#d8c8ff   — 10
#ede5ff   — 11
...
tokens: {
  radius: {
    ...
    true: 12,  // softer/rounder than Volta — dating apps feel warmer
  },
},
```
...
animations: {
  ...
  easings: {
    ...
    spring: 'cubic-bezier(0.22, 1.6, 0.36, 1)',  // bouncier — playful feel
  },
},
```
...
## Part 3 — Mock Data (`src/data/mock.ts`)
...
### Mock Pod (8 members — the current week's pod)
```ts
...
export interface MockPod {
  id: string
  name: string              // e.g. "Week 12 — Starlight Pod"
  startDate: string
  endDate: string
...
## Part 4 — App Layout (`src/layouts/AppShellLayout.tsx`)
...
## Part 5 — Pages (Full Implementation)
...
### 5a. Home Page (`/`) — `HomePage.tsx`

The pod dashboard / landing page:
- **Pod countdown**: "3 days left in this pod" with a progress-style indicator
- **Pod name**: "Week 12 — Starlight Pod"
- **Quick stats row**: "8 members · Day 4 of 7 · 2 mutual matches"
- **Today's activity cards**: Preview of today's ice-breaker and quiz (tappable → navigate to detail)
- **Pod member grid**: 2x4 grid of avatar circles with names, match status indicators
- **CTA**: "See today's ice-breaker" button

### 5b. Pod Detail Page (`/pod`) — `PodPage.tsx`

Full pod view:
- **Header**: Pod name, countdown timer, member count
- **Member grid** (larger cards): Photo, name, age, match status badge, tap → `/pod/member/:id`
...
### 5c. Ice-Breaker Page (`/pod/icebreaker`) — `IceBreakerPage.tsx`
...
### 5d. Quiz Page (`/pod/quiz`) — `QuizPage.tsx`
...
### 5e. Pod Member Profile (`/pod/member/:id`) — `PodMemberPage.tsx`
...
### 5f. Messages Page (`/messages`) — `MessagesPage.tsx`

DM conversation list:
- List of active conversations with pod members
- Each row: Avatar, name, last message preview, timestamp, unread dot
- Empty state: "No messages yet — start a conversation with your pod!"
- Tap → `/messages/:id`

### 5g. DM Thread (`/messages/:id`) — `DMThreadPage.tsx`
...
### 5h. Connections Page (`/connections`) — `ConnectionsPage.tsx`
...
- Empty state: "No connections yet — finish this week's pod to find your match!"
- Tap → `/connections/:id`

### 5i. Connection Profile (`/connections/:id`) — `ConnectionProfilePage.tsx`
...
### 5j. Week Review (`/week-review`) — `WeekReviewPage.tsx`
...
- **Post-submit view**: "You matched with X people! 🎉" celebration

### 5k. Profile Page (`/profile`) — `ProfilePage.tsx`
...
- **Stats card**: "12 likes received · 3 mutual matches · Profile 85% complete"

### 5l. Preferences Page (`/preferences`) — `PreferencesPage.tsx`

Matching preferences:
- **Age range**: Dual-handle slider (or two number inputs) with "21 – 35" display
...
### 5m. Onboarding Page (`/onboarding`) — `OnboardingPage.tsx`

Multi-step flow (standalone, NOT wrapped in AppShell — same as Volta):

1. **Welcome**: "Welcome to Crushd 💜" hero, tagline, illustration area, "Get Started" button
2. **Basic Info**: Name, birthday (age), gender identity, location — form fields
...
### 5n. Settings Page (`/settings`) — `SettingsPage.tsx`
...
3. **`GetProps<>` bug**: Define props from scratch — never use `GetProps<>` intersections
...
- [ ] Mobile-responsive layout (sidebar → drawer + bottom tabs)
